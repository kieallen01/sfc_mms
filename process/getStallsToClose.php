<?php
	...
	$cmdGetStallsToClose = $conn->query($strGetStallsToClose);
	$arrayGetStallsToClose = [];

	while ($data=$cmdGetStallsToClose->fetch(PDO::FETCH_BOTH)) {
		$applicantid 		= $data[0];
		...
		array_push($arrayGetStallsToClose, $rows);
	}
	echo json_encode($arrayGetStallsToClose);
