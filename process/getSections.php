<?php
	...
	";
	$cmdGetSections = $conn->query($strGetSections);
	$arrayGetSections = [];
	while ($data=$cmdGetSections->fetch(PDO::FETCH_BOTH)) {
		...
		array_push($arrayGetSections,$rows);
	}
	echo json_encode($arrayGetSections);
?>