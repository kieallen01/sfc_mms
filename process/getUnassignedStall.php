<?php
	...
	$strGetUnassignedStall = "
		...
		WHERE
			tbl_stall_application.fld_stall_application_status = '1'
		";
	
	$cmdGetUnassignedStall = $conn->query($strGetUnassignedStall);
	$arrayGetUnassignedStall = [];

	while ($data=$cmdGetUnassignedStall->fetch(PDO::FETCH_BOTH)) {
		$applicantid  = $data[0];
		...
		array_push($arrayGetUnassignedStall,$rows);
	}
	echo json_encode($arrayGetUnassignedStall);
?>