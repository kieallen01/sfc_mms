<?php
	...
	$cmdGetStallForClosure = $conn->query($strGetStallForClosure);
	$arrayGetStallForClosure = [];

	while ($data=$cmdGetStallForClosure->fetch(PDO::FETCH_BOTH)) {
		$applicantid 		= $data[0];
		...
		array_push($arrayGetStallForClosure, $rows);
	}
	echo json_encode($arrayGetStallForClosure);
